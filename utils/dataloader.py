...
class TrainDataLoader(AbstractDataLoader):
    ...
    def _create_sparse_matrix(self, df_feat, source_field, target_field, form='coo', value_field=None):
        ...
        ################################
        # path = '/home/gutang/A-Conf/data/clothing/mm_adj_5.pt'
        # adj_temp = torch.load(path).cpu()
        # adj_temp = adj_temp.coalesce()
        # indices = adj_temp.indices()
        # values = adj_temp.values() 
        # row = indices[0, :].numpy().astype(np.int32)
        # col = indices[1, :].numpy().astype(np.int32)
        # values = values.numpy().astype(np.float32)
        # print(values.shape)
        # print(row.shape)
        # print(col.shape)
        
        # src = np.concatenate([src,row],axis=0)
        # tgt = np.concatenate([tgt,col],axis=0)
        #######################
        ...
        #############
        # num_nodes = self.dataset.user_num + self.dataset.item_num
        # mat_1 = coo_matrix((data, (src, tgt)), shape=(num_nodes, num_nodes))
        # print(mat_1.shape)
        ############
