...
class CleanMR(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        # self.mlp_t = nn.Sequential(nn.Linear(self.dim_latent, self.dim_latent),
        #                            nn.LayerNorm(self.dim_latent),
        #                            nn.Linear(self.dim_latent, self.dim_latent),
        #                            nn.LayerNorm(self.dim_latent),
        #                            nn.Linear(self.dim_latent, self.dim_latent),
        #                            nn.LayerNorm(self.dim_latent))
        # self.mlp_v = nn.Sequential(nn.Linear(self.dim_latent, self.dim_latent),
        #                            nn.LayerNorm(self.dim_latent),
        #                            nn.Linear(self.dim_latent, self.dim_latent),
        #                            nn.LayerNorm(self.dim_latent),
        #                            nn.Linear(self.dim_latent, self.dim_latent),
        ...
        #                                       nn.LayerNorm(self.dim_latent),
        #                             nn.Linear(self.dim_latent,self.dim_latent))
        # self.fusion_layer = nn.Linear(self.dim_latent, 3)
        ...
        # self.FreeIB_item_model = FreeIBModel(self.dim_latent,self.preference,num_user,self.device)
    ...
    # def fusion_att(self,com,t_spe,v_spe):
    #     att_score = self.fusion_layer(com + t_spe + v_spe)
    #     att_score = torch.softmax(att_score,dim=-1)
    #     res = com * att_score[:,0].unsqueeze(-1) + t_spe * att_score[:,1].unsqueeze(-1) + v_spe * att_score[:,-1].unsqueeze(-1)
    #     return res
    def forward(self):
        ...
        # modal_feat = torch.cat([(v_common + t_common)/2, t_specific, v_specific],dim=-1)
        modal_feat = (v_common + t_common)/2 + t_specific + v_specific
        # modal_feat = v_common + t_specific + v_specific
        # modal_feat = 
        # modal_feat = t_feat + v_feat
        # modal_feat = self.fusion_att(v_common, t_specific, v_specfic)
        # modal_feat = torch.cat([v_common, t_specific, v_specfic],dim=-1)
        # modal_feat = torch.dropout(modal_feat,p=0.5,train=self.training)
        # modal_feat = torch.cat([t_feat , v_feat],dim=-1)
        # modal_feat = (t_common + v_common)/2 + t_specific + v_specfic
        # preference = torch.cat([self.t_preference, self.v_preference],dim=-1)
        user_embed, item_embed = self._gcn_pp(modal_feat, self.preference, self.ui_graph, self.iu_graph,norm=True)
        # h = item_embed
        # item_res = h
        # for _ in range(self.n_layers):
        #     h = torch.sparse.mm(self.mm_adj, h)                                         
        #     h = torch.dropout(h,p=0.6,train=self.training)
        # item_embed_temp = item_embed
        ...
        for _ in range(1):
            ...
            # item_embed = item_embed + F.normalize(item_h)
            user_embed = user_embed + F.normalize(user_h)
        
        # item_embed = item_embed + F.normalize(h)
        # user_embed = user_embed + F.normalize(user)
        ...
        # item_embed,item_ib_loss = self.FreeIB_item_model(item_embed)
        # print('ib_loss:',ib_loss)
        # item_embed = torch.dropout(item_embed,p=0.6,train=self.training)
        # user_embed = torch.dropout(user_embed,p=0.3,train=self.training)
        return user_embed, item_embed, club_loss, 1 * rec_loss, 0.01 * ib_loss
        # return user_embed, item_embed, 0, 0
        
        
        
        
        # # print("t_club_loss:",t_club_loss)
        # # print("v_club_loss:",v_club_loss)
        # # print("rec_loss:",rec_loss)
        # # print("/n")
        # # print(rec_loss)
        
        # # modal_feat = torch.cat([(t_common + v_common)/2, t_specific , v_specfic],dim=-1)
        # modal_feat = torch.cat([t_feat,v_feat],dim=-1)
        # # modal_feat = (t_common + v_common)/2 + t_specific + v_specfic
        # preference = torch.cat([self.t_preference, self.v_preference],dim=-1)
        # user_embed, item_embed = self._gcn_pp(modal_feat , preference, self.ui_graph, self.iu_graph,norm=True)
        # h = item_embed
        # # item_res = h
        # for _ in range(self.n_layers):
        #     h = torch.sparse.mm(self.mm_adj, h)                                         
        ...
        # t_feat = t_feat * t_score + (1-t_score) * temp_user
        # v_feat = v_feat * v_score + (1-v_score) * temp_user
        ...
        # item_rep = item_rep + h 
        # item_rep = torch.dropout(item_rep,p=0.6,train=self.training)
        
        # # user_rep = torch.dropout(user_rep,p=0.2, train=self.training)
        # return user_rep, item_rep,  0 ,0
    ...
    def bpr_loss(self, interaction):
        ...
        # print("mf loss:",mf_loss)
        # print("club_loss:",club_loss * 0.00)
        # print("rec_loss:",rec_loss * 0.0)
        if loss.item() > 50:
            print("mf_loss:",mf_loss.item())
            print("emb_loss:",emb_loss.item())
            print("club_loss:",1e-4 * club_loss.item())
            print("rec_loss:",rec_loss.item())
            print("ib_loss:", 0.1 * ib_loss.item())
            print("\n")
        return loss
...
class CLUB(nn.Module):
    ...
    def eval_mi(self, x_com, x_spe):
        ...
        # print("positive",positive.sum(dim = -1).mean())
        # print("negative",negative.sum(dim = -1).mean())
    ...
    def compute_club_loss(self, x_com, x_spe,noise_scale = 1):
        ...
        # negative_log_likelihood = -(((shuffled_x_spe - mu)**2 / (logvar.exp() + 1) + logvar)[indices].sum(dim=1).mean(dim=0) + 20)
        # noise = torch.normal(mean=0, std=1, size=x_spe.size()).to(shuffled_x_spe.device)
        # noise_scale = 1  # 控制噪声范围
        ...
        # print("x_spe - mu",(x_spe - mu).sum(dim=1).mean(dim=0))
        # print("logvar.exp()",logvar.exp().sum(dim=1).mean(dim=0))
        club_loss = log_likelihood - 2 * negative_log_likelihood
        # club_loss = log_likelihood + 20
        # print("log_likelihood:",log_likelihood)
        # print("negative_log_likelihood:",negative_log_likelihood)

        # 返回 CLUB Loss 和正则化项
        # variance_regularization = torch.abs((x_spe - mu)).mean() 
        total_loss = club_loss 
        # cosine_similarity = torch.nn.functional.cosine_similarity(x_com, x_spe, dim=1)
        # print("Mean Cosine Similarity:", cosine_similarity.mean().item())
        return total_loss 
...
class Corss_Rec(nn.Module):
    ...
    #     text_rec_feature = self.text_rec(torch.cat([image_common, text_specific],dim=-1))
    #     text_rec_loss = self.cal_rec_loss(text_rec_feature, text_origin)
    #     image_rec_feature = self.image_rec(torch.cat([text_common, image_specific],dim=-1))
    #     image_rec_loss = self.cal_rec_loss(image_rec_feature,image_origin)
    #     rec_loss = text_rec_loss + image_rec_loss
    #     return rec_loss
    def forward(self,text_common, text_specific, text_origin, image_common, image_specific, image_origin):
        tgt_text = image_common + text_specific
        # text_feat = self.text_linear(tgt_text)
        # text_feat = self.ln(text_feat) + text_feat
        # text_rec_feature = self.text_rec(text_feat)
        text_rec_loss = self.cal_rec_loss(tgt_text, text_origin) 
        
        tgt_image = text_common + image_specific
        # image_feat = self.image_linear(tgt_image)
        # image_feat = self.ln(image_feat) + image_feat
        # image_rec_feature = self.image_rec(image_feat)
        ...
        cosine_similarity_spe = torch.nn.functional.cosine_similarity(text_common, text_specific, dim=1).mean()
        cosine_similarity_t_spe = torch.nn.functional.cosine_similarity(image_common, image_specific, dim=1).mean()
        # print("Mean Cosine Similarity:", cosine_similarity.item())
        # print("Spe t Mean Cosine Similarity:", cosine_similarity_spe.item())
        # print("Spe v Mean Cosine Similarity:", cosine_similarity_t_spe.item())
...
class IBModel(nn.Module):
    ...
    def cal_ib_loss(self,x):
        ...
        # kl_loss = self.kl_loss(z,x)
        # kl_loss = self.kl_uniform_loss(random_z)
        kl_loss = self.kl_divergence(mu, std)
        # print("kl_loss: ",kl_loss)
        # kl_loss = - torch.sum(1 + logvar - mu.pow(2) - logvar.exp(), dim=1).mean()
        return kl_loss,z
    ...
    def kl_uniform_loss(self,z, a=-0.2, b=0.2):
        """
        计算 p(z|x) 和先验分布 r(z) = U(a, b) 的 KL 散度
        """
        # 先验分布密度
        prior_density = 1 / (b - a)  # r(z) 的概率密度
        # 后验分布密度 (假设 p(z|x) 的支持区间也为 [a, b])
