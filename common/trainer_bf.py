   ...
   and some other features helpful for model training and evaluation.
    ...
    def _obtain_modal_score(self, config, epoch_idx):
        ...
        if epoch_idx%1 == 0 and epoch_idx < 100 and epoch_idx != 0:
            ...
            print("t_score:",t_score[1400:1500])
            print("t_score max:",t_score.max())
            print("t_score max than {}:".format(self.text_threshold),t_score[t_score>self.text_threshold].shape)
            print("v_score:",v_score[1400:1500])
            print("v_score max:",v_score.max())
            print("v_score max than {}:".format(self.image_threshold),v_score[v_score>self.image_threshold].shape)
            # np.save(root_path+ "/t_score_after_" + str(epoch_idx) + ".npy", t_score)
            # np.save(root_path+ "/v_score_after_" + str(epoch_idx) + ".npy", v_score)
            np.save(root_path_2+ "/t_score_rmr_" + str(epoch_idx) + ".npy", t_score)
            np.save(root_path_2+ "/v_score_rmr_" + str(epoch_idx) + ".npy", v_score)
            ...
            np.save(root_path_2+ "/user_embed_rmr_" + str(epoch_idx) + ".npy", user_numpy)
            np.save(root_path_2+ "/item_embed_rmr_" + str(epoch_idx) + ".npy", item_numpy)
    ...
    def fit(self,config, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            # if epoch_idx == 1:
                ...
                print("replace image:",image_score.shape)
                print("replace text:",text_score.shape)
            ...
            # if epoch_idx % 10 == 0 and epoch_idx!=0 and ts_cnt < 3:
            #     print("beging replace >>>>>>>>>>>>>>")
            #     ts_dataloader, th_vl_dataloader, tl_vh_dataloader = self._build_ts_dataloader(config)
            #     VQGAN_model.train(ts_dataloader, config)
            #     th_vl_pred = VQGAN_model.inference(th_vl_dataloader, data_type="th_vl_data")
            #     tl_vh_pred = VQGAN_model.inference(tl_vh_dataloader, data_type="tl_vh_data")
            #     th_vl_index, tl_vh_index = self.group_index[1], self.group_index[2]
            #     self._replace_modal_feat(th_vl_pred, th_vl_index, tl_vh_pred, tl_vh_index)
            #     self.model.update_modal_feat(self.t_feat, self.v_feat)
            #     ts_cnt += 1
            
                # self.model._reset_paramaters()
                # self._build_optimizer()
                
            self._obtain_modal_score(config, epoch_idx)
